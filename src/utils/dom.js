...
export function *walk(start, limiter) {
	...
	while (node) {
		yield node;
		console.log("After Yield", node);
		if (node.childNodes.length) {
			node = node.firstChild; //poi rincomincia e urla "NOOOODE"
...
export function nodeAfter(node, limiter) {
	...
	if (after.nextSibling) {
		...
		while (after) { //Torna indietro finché non trova un parentNode con un sibiling 
...
export function rebuildAncestors(node) {
	console.log("rebuildAncestors", node.parentNode);
	let parent, ancestor;
	let ancestors = [];
	let added = [];
	...
	let element = node;
	while(element.parentNode && element.parentNode.nodeType === 1) { //NodeType 1 element node
		ancestors.unshift(element.parentNode); //The unshift() method adds one or more elements to the beginning of an array and returns the new length of the array.
		element = element.parentNode; // aggiunge gli ancestor fintanto che ci sono nodi a salire
		
	}
	console.log("ancestors",ancestors);
	for (var i = 0; i < ancestors.length; i++) {
		ancestor = ancestors[i];
		parent = ancestor.cloneNode(false); //Non copia i child nodes.
